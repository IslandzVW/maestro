...
from inworldz.maestro.MaestroLogging import log

import thread
import threading

MAESTRO_API_TASK_STATUS_TYPE = ['pending', 'success', 'failure']

class MaestroTask(threading.Thread):
    """Represents a Asynchronous Task used by MAESTRO API.
    ...
    def __init__(self, uuid, func, args, func_name, return_type, label, desc,
        ...
        @type uuid: string
        @param func: Method to call (from MaestroAPI)
        ...
        self.status = MAESTRO_API_TASK_STATUS_TYPE[0]
    ...
    def run(self):
        ...
        try:
            ...
            if result['Status'] == 'Success':
                ...
                self.set_status(MAESTRO_API_TASK_STATUS_TYPE[1])
            else:
                self.error_info = result['ErrorDescription']
                self.set_status(MAESTRO_API_TASK_STATUS_TYPE[2])                
        except Exception, e:
    ...
    def get_record(self):
        """Returns a MAESTRO API compatible record."""
        return {
            'uuid': self.uuid,            
    ...
    def get_progress(self):
        ...
        if self.status != MAESTRO_API_TASK_STATUS_TYPE[0]:
            return 100
    ...
    def log_progress(cls, progress_min, progress_max,
        ...
        This is a class method so other parts of Maestro will update
        the task progress by calling:

        MaestroTask.push_progress(progress_min, progress_max,
                               func, *args, **kwds)
